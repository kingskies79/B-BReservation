...
export class ImageUploadComponent {
  ...
  constructor(private toastr: ToastrService,  private imageService: ImageUploadService) {
  }
    ...
    if ((file = event.target.files[0]) && (file.type === 'image/png' || file.type === 'image/jpeg')) {
      ...
      img.onload = function () {
        ...
        if (this.width > FileSnippet.IMAGE_SIZE.width && this.height > FileSnippet.IMAGE_SIZE.height) {
          ...
          self.toastr.error(`Minimum width is ${FileSnippet.IMAGE_SIZE.width}
          and minimum heigth is ${FileSnippet.IMAGE_SIZE.height}`, 'Error!');
        }
      }
    ...
    if (this.selectedFile) {
     ...
     // console.log('this.selectedFile ' + this.selectedFile);
      reader.addEventListener('load', (event: any) => {
       ...
       // console.log('this.selectedFile.file.name ' + this.selectedFile.file);
