    ...
    Rental.findById(rental._id)
        ...
        .exec(async function(err, foundRental) {
            ...
            if (isValidBooking(booking, foundRental)) {
                debugger;
                console.log('foundRental ' + foundRental);
                booking.user = user;
                ...
                if (payment) {
                    ...
                    booking.save((err) => {
                        if (err) {
                            return res.status(422).send({ errors: normalizeErrors(err.erros) });
                        }
                        foundRental.save();
                ...
                return res.status(422).send({ errors: [{ title: 'Invalid Booking', detail: 'Chooses dates are already taken!' }] });
            }
        })
